...
void ADC0_IRQHandler(){
	uint16_t analog_input = (uint16_t) ADC0->R[0];
		float measured_lux = analog_input * 6000 / 65535; //- 64900;/// ;
	
	if( measured_lux < 2000)
		led_state_adc = 0;
	else if(measured_lux < 4000)
		led_state_adc = 1;
	else
		led_state_adc = 2;
	
		char buffer[8];
		itoa(measured_lux, buffer, 8);
		UART0_Transmit_String("Lux: ");
